...
- [布局](#布局)
  - [水平盒布局（QHBoxLayout）](#水平盒布局qhboxlayout)
    - [设置控件的对齐方式](#设置控件的对齐方式)
  - [垂直盒布局](#垂直盒布局)
  - [设置布局的伸缩量（addStretch）](#设置布局的伸缩量addstretch)
  - [让按钮永远在窗口的右下角](#让按钮永远在窗口的右下角)
  - [栅格布局：](#栅格布局)
  - [表单布局](#表单布局)
...
+ 为类添加多个信号
+ 信号和槽的 N对N的连接
  * 同一个信号绑定多个槽，会根据 槽的绑定顺序，依次触发
+ 为窗口类添加信号
+ 多线程更新UI数据
  * 在两个线程中传递数据
+ 信号与槽自动连接
  * 槽函数的规定
    - on_ogjectname_signalname
+ 使用Lambda表达式为槽函数传递参数
...
+ 多窗口交互
  ...
  * （2）弱耦合 ：使用信号和槽
    ...
    - 例如：如果A直接访问B窗口的控件，一旦B窗口控件发生改变，那么A和B的代码都需要改变
